...
// TripCreateModal과 동일한 오버레이+카드 패턴 — 계획을 전체공개로 바꿀 때 한 줄 코멘트를 받는다.
// 비워서 게시해도 되고(건너뛰기), 게시한 뒤에는 헤더의 코멘트를 제목처럼 더블클릭해서 바로 고칠 수 있다.
export default function PublishCommentModal({ initialComment = '', onClose, onSubmit }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
      <div className="w-full max-w-[380px] rounded-2xl bg-surface p-6 shadow-popup" onClick={(e) => e.stopPropagation()}>
        <div className="mb-1 flex items-center justify-between">
          <h2 className="text-[16px] font-bold text-slate-800">전체공개로 바꿀까요?</h2>
        ...
        <p className="mb-4 text-[12.5px] text-slate-500">
          여행자 피드에 제목 아래로 보여줄 한 줄 코멘트를 남겨보세요. 선택 사항이고, 게시한 뒤에도 제목처럼 더블클릭해서 다시 고칠 수 있어요.
        </p>
