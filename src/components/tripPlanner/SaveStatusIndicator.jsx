...
const CONFIG = {
  ...
  en: {
    saving: { icon: 'mdi:loading', spin: true, text: 'Saving', tone: 'border-slate-200 text-slate-400' },
    saved: { icon: 'solar:check-circle-bold', spin: false, text: 'Saved', tone: 'border-emerald-100 text-emerald-600' },
    error: { icon: 'solar:danger-triangle-bold', spin: false, text: 'Save failed · reverted', tone: 'border-rose-100 text-rose-500' },
  },
}
...
export default function SaveStatusIndicator({ status }) {
  ...
  const { icon, spin, text, tone } = (CONFIG[language] ?? CONFIG.en)[status]
