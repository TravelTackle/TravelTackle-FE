...
const T = {
  ko: {
    checklist: ['여행 취향 저장 완료', '맞춤 추천 준비 완료', '언제든 설정 변경 가능'],
    completeLabel: '취향 설정 완료',
    title: '모든 준비가 끝났어요!',
    subtitle: (name) => (
      <>
        {name}님의 취향을 바탕으로
  ...
  en: {
    checklist: ['Travel preferences saved', 'Personalized picks ready', 'Change your settings anytime'],
    completeLabel: 'Preferences saved',
    title: 'All set!',
    subtitle: (name) => (
      <>
        Based on {name}&apos;s preferences,
        <br />
        personalized feedback and recommendations start now.
      </>
    ),
    startTrip: 'Start traveling',
  },
}
  ...
  return (
    <OnboardingHeader>
      <div className="mx-auto flex min-h-[500px] max-w-[500px] flex-col items-center justify-center text-center">
        ...
        <p className="mt-7 text-[13px] font-bold text-brand">{copy.completeLabel}</p>
        <h1 className="mt-2 text-[32px] font-extrabold leading-[1.2] tracking-tight text-slate-900 sm:text-[36px]">
          {copy.title}
        </h1>
        <p className="mt-4 text-[15px] leading-relaxed text-slate-500">{copy.subtitle(user?.name)}</p>
